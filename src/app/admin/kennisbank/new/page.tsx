...
export default function NewKennisbankPage() {
  ...
  async function save(overrideStatus?: "draft" | "published") {
    ...
    router.replace(`/admin/kennisbank/${article.id}`);
  }
  ...
  async function runAiSeo() {
    ...
    const res = await fetch("/api/kennisbank/ai-seo", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ title, content, categoryName: selectedCat?.name ?? "" }),
    });
    setAiLoading(false);
          ...
          <div className="bg-white rounded-2xl border border-[#E8E4DE] p-4">
            ...
            <div className="flex flex-col gap-1.5 mb-2">
              {relatedArticles.map(slug => (
                <div key={slug} className="flex items-center gap-2 group">
                  <span className="flex-1 text-[11px] text-[#6B5E54] font-mono truncate">{slug}</span>
                  <button type="button" onClick={() => setRelatedArticles(r => r.filter(x => x !== slug))}
